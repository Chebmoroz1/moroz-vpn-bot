...
class TrafficManager:
    ...
    def get_traffic_stats_by_period(
        ...
        ВАЖНО: Использует только последний snapshot для каждого ключа,
        так как snapshots содержат накопительные значения.
        ...
        try:
            ...
            # Находим последний snapshot для каждого ключа за период
            subquery = db.query(
            ...
            if vpn_key_id:
                subquery = subquery.filter(TrafficStatistics.vpn_key_id == vpn_key_id)
            
            subquery = subquery.group_by(TrafficStatistics.vpn_key_id).subquery()
            
            # Берем только последние snapshots для каждого ключа
            query = db.query(
                VPNKey.id.label('vpn_key_id'),
                VPNKey.key_name,
                VPNKey.user_id,
                TrafficStatistics.bytes_received.label('total_received'),
                TrafficStatistics.bytes_sent.label('total_sent'),
                TrafficStatistics.last_connection.label('last_connection'),
                TrafficStatistics.connection_ips.label('connection_ips')
            ).join(
                subquery,
            ...
            # Фильтруем по ключу, если указан (уже применен в subquery, но для безопасности)
            if vpn_key_id:
    ...
    def _get_chart_data_from_snapshots(
        ...
        try:
            ...
            try:
                ...
                # Получаем начальные значения для каждого ключа
                # Ищем последний snapshot ДО начала периода для каждого ключа
                # Если такого нет, используем первый snapshot в периоде как базовый (но не считаем его трафик)
                initial_key_data = defaultdict(lambda: {'received': 0, 'sent': 0})
                first_snapshot_timestamps = {}  # Сохраняем timestamp первого snapshot для каждого ключа
                
                # Находим уникальные ключи
                unique_key_ids = set(row.vpn_key_id for row in results)
                ...
                for key_id in unique_key_ids:
                    ...
                    if first_snapshot_before:
                        ...
                        initial_key_data[key_id] = {
                            'received': first_snapshot_before.bytes_received or 0,
                            'sent': first_snapshot_before.bytes_sent or 0
                        }
                    else:
                        # Если нет snapshot'а до начала периода, находим первый в периоде
                        # и используем его как базовый (чтобы не считать весь накопленный трафик)
                        first_snapshot_in_period = db.query(TrafficStatistics).filter(
                            TrafficStatistics.vpn_key_id == key_id,
                            TrafficStatistics.timestamp >= start_datetime
                        ).order_by(TrafficStatistics.timestamp.asc()).first()
                        
                        if first_snapshot_in_period:
                            # Сохраняем первый snapshot как базовый (чтобы не считать его трафик)
                            initial_key_data[key_id] = {
                                'received': first_snapshot_in_period.bytes_received or 0,
                                'sent': first_snapshot_in_period.bytes_sent or 0
                            }
                            # Запоминаем timestamp первого snapshot (чтобы пропустить его в расчете)
                ...
                while current_interval <= end_datetime:
                    ...
                    # Если есть snapshots для этого интервала, вычисляем разницу
                    if interval_snapshots:
                        # Для каждого ключа в интервале вычисляем разницу
                        for key_id, snapshot_data in interval_snapshots.items():
                            # Проверяем, не является ли это первым snapshot'ом для этого ключа
                            # (если snapshots начали собираться недавно, первый snapshot содержит весь накопленный трафик)
                            is_first_snapshot = (
                            ...
                            if is_first_snapshot:
                                # Пропускаем первый snapshot - он содержит весь накопленный трафик
                                # Просто обновляем предыдущие значения без добавления трафика
                        ...
                        # Если нет snapshots для этого интервала, трафик = 0
                        # (это нормально, если snapshot еще не был создан)
                        interval_received = 0
                        interval_sent = 0
    ...
    def get_users_traffic_stats(
        ...
        try:
            ...
            try:
                ...
                # Находим последний snapshot для каждого ключа за период
                # ВАЖНО: Используем только последние snapshots, так как они содержат накопительные значения
                subquery = db.query(
                    TrafficStatistics.vpn_key_id,
                    func.max(TrafficStatistics.timestamp).label("max_timestamp")
                ).filter(
                ...
                # Получаем статистику по пользователям (только последние snapshots)
                query = db.query(
                    VPNKey.user_id,
                    ...
                    VPNKey, TrafficStatistics.vpn_key_id == VPNKey.id
                ).filter(
